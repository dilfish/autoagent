...
| minSdk 24 + targetSdk/compileSdk 36 | min 只决定"能装多老的手机"（24=Android 7.0，2016 年以后全覆盖），target 决定"在新系统上的行为适配"，两者解耦。项目所用能力（dispatchGesture 的最低要求就是 24、无障碍/Compose/OkHttp/sshj）全部在 24 可用，无功能损失；v0.5.0 曾误把 min 升到 35，会导致 Android 15 以下装不上，已改回 |
